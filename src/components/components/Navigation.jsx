      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="hidden md:flex fixed inset-0 z-40 bg-background flex-col overflow-hidden"
          >
            <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-primary" />

            <div className="flex-1 max-w-7xl mx-auto w-full px-6 md:px-14 flex flex-col justify-between overflow-y-auto pt-20 pb-6">
              <nav className="flex flex-col mt-0">
                {navIds.map((id, i) => (
                  <motion.button
                    ...
                    onClick={() => scroll(id)}
                    className={`group flex items-center gap-4 py-3.5 border-b border-border/30 text-left hover:pl-2 transition-all duration-300 last:border-0 ${activeSection === id ? "pl-2" : ""}`}
                  >
