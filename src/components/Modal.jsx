...
const Modal = ({ modalId, isOpen, onClose }) => {
    ...
    const modalData = {
        ...
        myModal2: {
            ...
            content: (
                <>
                    <h2>🛍️ downtownastana.com — Laravel e-commerce (Docker, Make, Filament)</h2>
                    <p>
                        Высокотехнологичный интернет-магазин национальных аксессуаров, реализованный на <strong>Laravel</strong>
                        (production-ready конфигурация). Проект фокусируется на скорости отклика, надёжности развёртывания и удобстве администрирования.
                    </p>

                    <p><strong>Ключевые технические решения:</strong></p>
                    <ul style={{ color: '#000', paddingLeft: '20px', marginTop: '10px', listStyle: 'disc' }}>
                        <li>
                            <strong>Контейнеризация:</strong> полная изоляция среды через <strong>Docker</strong> и <strong>Docker Compose</strong>;
                            все сервисы (PHP-FPM, Nginx, БД, Redis, очередь) описаны в compose и управляются через Makefile.
                        </li>
                        <li>
                            <strong>Автоматизация:</strong> Makefile для типичных операций (сборка, миграции, сиды, очистка кешей, деплой);
                            CI/CD интеграция (pipeline для тестов, миграций и деплоя).
                        </li>
                        <li>
                            <strong>Админ-панель:</strong> Filament для управления каталогом, заказами и пользователями; тонкая настройка ролей/прав и middleware (учтены кейсы production, в т.ч. проверка APP_ENV и разрешений).
                        </li>
                        <li>
                            <strong>Хранение и медиа:</strong> оптимизированная работа с файлами — <em>storage:link</em>, поддержка S3-совместимого хранилища, ленивое преобразование и оптимизация изображений.
                        </li>
                        <li>
                            <strong>Фоновые задачи и масштабируемость:</strong> очереди задач (Redis/queue workers), supervisor / systemd для воркеров, мониторинг задач и ретраев.
                        </li>
                        <li>
                            <strong>Производительность:</strong> кеширование (Redis), сжатие и CDN-подготовка медиа, оптимизированные SQL-миграции и индексы для ключевых таблиц.
                        </li>
                        <li>
                            <strong>Надёжность:</strong> кастомные Artisan-команды для регламентных операций, централизованный лог-менеджмент и корректная обработка прав на storage/logs для production.
                        </li>
                        <li>
                            <strong>Тестирование и качество кода:</strong> набор unit/integration тестов, статический анализ и pre-commit hooks (gofmt/linters для инфраструктурных скриптов).
                        </li>
                    </ul>

                    <p style={{ marginTop: '10px', fontStyle: 'italic' }}>
                        В проекте реализованы практические решения для безопасного развёртывания и сопровождения: от CI/CD и контейнеризации до управления доступом в админке и надёжной работы фоновых задач.
                    </p>
                </>
            ),
        },
